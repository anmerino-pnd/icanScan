...
- **v1.2.0 Desktop Application Viewport System (`100vh`):** Native desktop-first layout architecture fitting 100% within the viewport height. Consolidates scanner setup, page addition, notebook metrics, quality options, clear session, and **Export All to PDF** into an organized 380px left sidebar, leaving the right workspace 100% dedicated to full-height page thumbnail grid management.
...
- **Responsive Overlapping Folder Navigation Tabs:** Connected folder tab navigation header that maintains seamless visual alignment across screen resolutions, featuring responsive collapse to centered icons (`Layers`, `Archive`, `Wrench`) on compact viewports to prevent wrapping or broken layouts.
- **Instant GPU-Accelerated Optical Studio with Smooth Transitions:** Dual-engine preview studio applying real-time browser GPU hardware filters (`brightness`, `contrast`, `rotation`, and `B&W binarization threshold`) at 60 FPS without UI block or network delay. Features a completely flicker-free navigation system using LQIP (Low Quality Image Placeholders) and native `key` remounting, plus full manual Zoom, Pan, and auto-Fit scaling with native CSS scrolling.
- **Interactive Visual Page Preview Engine in PDF Tools:** Real-time mini-photograph page previews rendered across all PDF tools:
  - **Extract PDF Pages to Images:** Live thumbnail gallery rendering exact pages matching range specifications (e.g., `1, 3, 5-9, 10` or `"todas"` / `"all"`) before extraction to PNG (Lossless) or JPG (150–600 DPI) with ZIP archiving.
  - **Multi-Range PDF Split & Extraction:** Visual split group cards rendering thumbnail page strips for each generated output PDF file (e.g., `Document_pages_1-3.pdf`, `Document_pages_4.pdf`, `Document_pages_5-10.pdf`).
  - **Images to PDF Union:** Interactive image grid with order index badges, reordering handles, and preview thumbnails.
  - **Merge / Combine Multiple PDFs:** Comprehensive document cards featuring horizontal scrollable page thumbnail strips for every loaded PDF file.
  - **Reorder & Rotate PDF Pages:** Full visual page grid with 90° rotation badges and drag-and-drop reordering.
- **Complete i18n & Dynamic Language Synchronization:** 100% UI language parity (including all PDF tools and Drive Compression views) with instant language toggle (`Español / ES` ↔ `English / EN`), dynamically converting range defaults (e.g. `"todas"` ↔ `"all"`) and sub-tool headings without losing active user inputs.
- **Precision Centered Icons & Styled Drilldown System:** Standardized flex container centering across all vector icons and hand-drawn drilldown action buttons (`.btn-secondary`, `.paper-card`, `.stamp-badge`), decoupled from micro-interactions to prevent CSS layout teleportation on hover/active states.
- **High-Performance PyMuPDF Thumbnail Caching:** Fast in-memory hash caching for rendered 100 DPI page thumbnails, enabling sub-10ms preview updates when modifying page range parameters.
