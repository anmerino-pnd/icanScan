...
- **Complete i18n & Dynamic Language Synchronization:** 100% UI language parity with instant language toggle (`Español / ES` ↔ `English / EN`), dynamically converting range defaults (e.g. `"todas"` ↔ `"all"`) and sub-tool headings without losing active user inputs.
- **Precision Centered Icons & Styled Drilldown System:** Standardized flex container centering across all vector icons and hand-drawn drilldown action buttons (`.btn-secondary`, `.paper-card`, `.stamp-badge`).
